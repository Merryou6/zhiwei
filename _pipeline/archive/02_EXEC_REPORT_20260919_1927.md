...
F-1（发现，非本迭代可修复）：ΔAccuracy 结构性为空
...
8.1 未完成（计划内已知边界，非缺陷）
  ...
  (2) ΔAccuracy 双侧数据（F-1）：前端已实现且如实展示，但演示路径下恒为 null；需协议层裁决后才有非空 Δ。
...
2) 10 页面（PRD §5）+ 端到端闭环（注册→自报→测评→试卷→归因→处方→对话→复测→报告→图谱）全部落地，
   最终一轮演示 10 步全绿（唯一保留项 F-1：ΔAccuracy 双侧数据为空，属冻结后端/协议层，如实上报）。
...
5) 需上游裁决的 1 项：F-1 ΔAccuracy 测量口径（建议在测量协议层解决，不在前端或冻结引擎内打补丁）。
