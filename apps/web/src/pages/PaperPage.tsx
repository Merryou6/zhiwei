...
export default function PaperPage() {
    ...
    return (
      <section className="max-w-3xl">
        ...
        <div className="mt-6 flex items-center gap-3">
          ...
          {!allMarked ? <span className="text-xs text-ink-soft">还有 {rows.filter((r) => r.result === null).length} 题没标</span> : null}
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="max-w-2xl">
            ...
            return (
              <li key={file.file_id}>
                ...
                >
                  ...
                  <p className="mt-1 text-xs text-ink-soft">{file.file_id}</p>
                </button>
              </li>
            );
          })}
      ...
      {status ? <p className="mt-3 text-xs text-ink-soft">{status}</p> : null}
    </section>
  );
}
